...
-메서드 내부에 또다른 메서드를 정의할 수 없다. 이 경우 에러가 발생함.
-반환값이 없다면 void를 명시해야 한다.
-함수명은 변수명과 동일한 규칙으로 작성한다.


Java에서 Naming Rule(이름 지을 때의 규칙)
-클래스명 : SimpleClass
...
/*
메서드 형태1] 매개변수X, 반환값X
	:둘 다 없는 형태의 메서드로, 주로 메뉴를 출력하거나 하는 단순한 기능을 가진다. */
public class E02MethodType01 {
	...
	static void returnError() {
		...
		/*
		이와 같이 메서드 중간에 return 을 기술하면 즉시 종료되므로 그 아래의 코드는 실행될 수 없는 상태가 된다.
		따라서 에러가 발생한다.
		이런 경우에는 반드시 조건문과 같이 기술해서 조건부로 실행될 수 있도록 코딩해야 한다.
		단 메서드의 마지막 부분에 쓰는 경우에는 조건문이 없어도 된다.
		 */
//		return;
