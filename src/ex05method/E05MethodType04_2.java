...
/*
시나리오] 여러 개의 숫자를 입력받은 후 최대값을 구하는 메서드를 정의하시오.
단, 숫자의 개수는 매개변수로 전달받고 숫자의 개수만큼 Scanner 클래스를 통해 입력받는다.
...
public class E05MethodType04_2 {
	...
	/*
	main()에서 전달한 인수의 크기만큼 반복해서 정수를 입력받은 후 최대값을 찾아서 반환하는 메서드 정의 */
	static int returnMaxNumber(int numberCnt) {
		...
		for(int i = 1; i<=numberCnt; i++) {
			...
			if(i == 1) {
				/*
				입력받은 정수 중 최대값을 찾아야하므로 첫번째 입력값을 비교하기 위한 기준값으로 설정한다. */
				maxVal = inputNum;
			}
			else {
				/*
				두번째 입력값부터 기존의 최대값과 비교해서 더 큰 값이 발견되면 교체한다.	 */
				if(maxVal < inputNum) {
					maxVal = inputNum;
				}
			}
