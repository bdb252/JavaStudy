...
public class SQLselect1 extends MyConnection{
	...
	: insert, update, delete 쿼리문을 실행할 때 사용.
	  쿼리실행 후 반환값은 정수로, 적용된 행의 개수가 반환된다.
	executeQuery()
	: select 쿼리문을 실행한다. 반환값은 조회할 레코드이므로 ResultSet인스턴스가 반환된다.
	  Java에서 반환받은 후 while문으로 개수만큼 반복하여 인출한다.
	 */
	@Override
	public void dbExecute() {
		try {
			stmt = con.createStatement();
			//레코드 전체를 조회하는 select쿼리문 작성
			...
			while(rs.next()) {
				...
				/* 오라클의 date타입을 getDate()로 인출하면 0000-00-00형식으로 출력된다.
				 * 또한 자료형이 Date이므로 java.sql패키지의 클래스를 사용해야 한다. */
				java.sql.Date regidate = rs.getDate("regidate");
				/* 날짜를 getString()으로 인출하면 시간까지 포함되어 출력된다. 
				0000-00-00 00:00:00형식이므로 적당한 크기로 잘라야 한다면 substring()을 사용한다. */
