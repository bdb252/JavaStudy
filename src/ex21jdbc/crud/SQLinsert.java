...
//DB연결을 위한 MyConection 클래스를 상속하여 정의
public class SQLinsert extends MyConnection{
	...
	/*
	이 멤버변수는 생성자에서 초기화되지 않는다. 생성자는 대부분 멤버변수의 초기화를 목적으로 정의되지만, 
	JDBC프로그래밍에서는 쿼리문을 실행해야 하므로 인스턴스가 생성된 후 변수를 초기화해야 한다. 
	 */
	
	//insert쿼리문 실행을 위한 메서드
	@Override
	public void dbExecute() {
		try {
			/*
			Statement 인터페이스
			: 정적 쿼리문을 실행할 때 사용한다. 정적 쿼리문이란, 인파라미터가 없는 완성된 쿼리문을 말한다.
			  수정은 개발자가 직접 진행해야 한다.
			 */
			stmt = con.createStatement();
			//쿼리문 작성
			query = "insert into member values ('test4', '4444', '테스터4', sysdate)";
			//insert의 반환값을 정수로 1이면 성공, 0이면 실패
//			result = stmt.executeUpdate(query);
			...
			: 인파라미터가 있는 동적쿼리문을 실행할 때 사용한다. 인파라미터는 ?로 표시하고,
			 인스턴스 생성 이후 setXX()메서드를 통해 설정한다. 인덱스는 1부터 시작한다.
			 */
