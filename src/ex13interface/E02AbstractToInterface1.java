...
/*
프로젝트 전체에서 기준의 역할을 하는 추상클래스. 해당 클래스에는 추상메서드를 포함하고 있으므로 
하위에서 상속받게 되면 반드시 추상메서드를 오버라이딩 해야 한다.
 */
...
/*
DTO(Data Transfer Object) : 데이터를 저장하고 전송하기 위한 용도의 객체를 의미한다. VO (Value Object)라고 표현하기도 한다.
...
class PersonalNumberStorageExt extends PersonalNumberStorageAbs{
	...
	/*
	상속을 통해 추상메서드를 포함하게 되므로 반드시 아래와 같이 오버라이딩 해야 한다. 이는 필수사항이며 지키지 않는다면 에러가 발생한다.
	 */
	@Override
	...
	public String searchPersonalInfo(String juminNum) {
		...
		for(int i = 0; i<numOfPerInfo; i++) {
			...
			if(juminNum.compareTo(personalArr[i].getJuminNum())==0) {
				...
				/*
				DTO의 멤버변수를 private으로 선언되었으므로 외부에서는 접근이 불가능하다. 따라서 getter를 통해 출력해야 한다.
				 */
			}
		}
