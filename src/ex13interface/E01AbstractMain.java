...
-추상클래스는 대부분 추상메서드를 포함하고 있으므로, 이를 상속받은 클래스는 반드시 추상메서드를 오버라이딩 해야한다.
...
/*
추상클래스2: 추상메서드를 포함한 클래스. 추상메서드를 멤버로 포함한 클래스는 반드시 abstract로 선언해야 한다.
그렇지 않으면 에러가 발생한다.
 */
...
/*
위 클래스를 상속하면 부모가 가진 추상메서드를 그대로 상속받게 되므로 해당 클래스도 abstract로 선언해야 한다.
단, 이 경우 해당 클래스로도 인스턴스를 생성할 수 없게 된다.
따라서 인스턴스 생성을 위해서는 부모의 추상메서드를 자식쪽에서 오버라이딩하면 된다.
 */
class AbsClassChild2 extends AbsClass2{
	/*
	오버라이딩은 부모쪽의 메서드를 가리고, 새로운 기능을 재정의하는 것이므로 해당 클래스는 추상메서드를 포함하지 않은 상태가 된다.
	따라서 인스턴스 생성이 가능해진다.
...
//		AbsClass1 absClass1 = new AbsClass1();
		...
		/*
		추상클래스는 인스턴스 생성을 할 수 없지만, 참조변수로는 사용할 수 있다.
		 */
		...
		////////////////////////////////////////////
		AbsClassChild2 abc2 = new AbsClassChild2();
		abc2.absMethod(10);
	}
