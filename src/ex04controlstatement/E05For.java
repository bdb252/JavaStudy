...
/*
for문
	: while문과 같은 반복문으로 , 초기값, 조건식, 증감식이 한 줄에 있어서 반복의 횟수를 명확히 알 수 있다.
	  따라서 반복의 횟수가 정해져 있는 경우에 주로 사용된다.
	 형식]
	 	for(초기값 ; 조건식 ; 증감식 ;){
	 		실행문장;
	 	}
...
public class E05For {
	...
	public static void main(String[] args) {
		...
		//변수 i가 1~5까지 1씩 증가해서 5번 반복
		for(int i = 1; i<=5; i++) {
			System.out.println("i="+i);
		}
		
		/*
		시나리오] 1~100까지의 합을 구하는 프로그램을 for문을 통해 작성하시오.
		 */
		//누적합 저장용 변수 선언
		int sum = 0;
		//초기값 1부터 100까지 반복
		...
		System.out.println("1~10 사이의 2의 배수의 합(방법1) : "+total);
		...
		System.out.println("1~10 사이의 2의 배수의 합(방법2) : "+total);
...
//			a++;
//			/*
//			무한루프는 기본적으로 에러가 발생된다. 그리고 해당 루프를 탈출하지 못한다면 
//			아래에 있는 모든 코드는 실행될 수 없는 코드가 되므로 "Unreachable code"라는 에러가 발생한다.
...
//		}
		...
		여기서 사용된 변수 j는 지역번수(Local variable)이라고 한다.
...
//		System.out.println("위 for문에서 선언한 변수 j = "+j);
		...
		/*
		for문의 초기값은 외부(넓은 지역, 여기서는 main지역)에서 선언하면 
		해당 변수는 for문 내부(좁은지역)에서 사용할 수 있다.
