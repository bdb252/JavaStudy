...
//		Vector<String> list = new Vector<String>();
		...
		객체 저장 : add()
			...
			add(인덱스, 인스턴스): 데이터 저장 시 인덱스를 직접 부여한다. 단, 인덱스를 건너뛰면 에러가 발생하므로 주의해야 한다.
...
//		list.add(5,"소방차");//에러발생
		...
		/*
		중복저장 : List는 배열의 특성을 가지므로 데이터를 중복저장할 수 있다. 동일한 데이터라도 index로 구분할 수 있기 때문이다.
		...
		/*
		삽입하기(끼워넣기)
			해당 인덱스에 인스턴스를 삽입한다. 기존 인스턴스는 뒤로 1칸씩 밀려난다. 즉 자동으로 인덱싱된다.
		 */
		...
		/*
		포함여부확인
			: 특정 인스턴스가 저장되어 있는지 확인할 때 사용. 존재한다면 true반환
		 */
		...
		/*
		방법3 : 인스턴스의 참조값을 통해 삭제한다. 이 경우에는 삭제에 성공하면 true가 반환된다.
