...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		...
		/*
		아래 2개의 컬렉션은 내부적인 자료구조만 다를 뿐 구현 방법에는 차이가 없다.
		인스턴스 생성 시 Student형의 타입매개변수를 통해 생성한다. Student를 저장할수 있는 상자가 된다.
		 */
...
//		ArrayList<Student> list2 = new ArrayList<Student>();
		...
		/*
		일반 for문 사용을 위해 size()메서드로 추가된 인스턴스의 개수를 반환받을 수 있다.
		...
		또한 동일한 인스턴스가 존재한다면 앞에서부터 순서대로 삭제된다. 즉 동시에 삭제되지 않는다.
		 */
		...
		/*
		시나리오] 컬렉션에 저장된 객체를 이름으로 검색하여 삭제할 수 있는 코드를 작성하시오. 이때 반복문은 확장 for문을 사용하시오.
		 */
		int idx = 0;
		...
		for(Student st : list2) {
			/*
			이름으로 검색 시 멤버변수 name은 private으로 선언되었으므로 getter을 통해 접근해야 한다.
			삭제를 위해 입력한 이름과 Student 인스턴스에 저장된 name과 일치하는지 확인한 후 해당 인스턴스를 검색할 수 있다.
			 */
		...
		인스턴스의 인덱스를 통해 삭제하면 참조값이 반환된다. 따라서 삭제된 인스턴스의 정보를 출력할 수 있다.
		 */
