...
/*
HashMap<K,V>
- Key, Valus의 쌍으로 인스턴스를 저장한다.
- Key 값은 중복될 수 없다. 만약 중복되면 기존의 데이터를 덮어쓰기 한다.
- Key값으로 검색되므로 다른 컬렉션에 비해 속도가 느리다.
- 저장 순서는 유지되지 않는다.
 */
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		...
		/*
		int는 String으로 형변환할 수 없으므로 타입에러가 발생한다.
		 */
		int number = 20;
//		map.put("age", number);
		...
		/*
		중복저장 : 기존에 입력된 name이라ㅣ는 Key가 있기 때문에 '홍길동'이 반환된다.
		그리고 기존의 값은 '최길동'으로 수정된다.
		 */
		System.out.println("name이라는 키값으로 저장된 이전의 값:"+map.put("name", "최길동"));
		//기존의 값을 덮어쓰기하므로 4개가 출력된다.
		System.out.println("키값으로 중복 저장후 개체수:"+map.size());
		...
		/*
			2. 키값을 모를 때(혹은 너무 많아서 명시하기 힘들 때
		...
		/*
		value만 얻어와야 할때 : values()메서드를 통해 값만 얻어올 수 있다.
		...
		/*
		객체의 존재유무 확인 : Map은 key와 value가 있으므로 메서드도 2개로 나눠져있다.
		 */
		...
		/*
		삭제 : Key를 통해 삭제하고, 삭제에 성공하는 해당 value가 반환된다.
		 */
