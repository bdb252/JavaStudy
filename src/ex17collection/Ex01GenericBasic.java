...
public class Ex01GenericBasic {
	...
	public static void main(String[] args) {
		...
		orange1.showInfo();
		/*
		상자 1은 오렌지 전용 박스이므로 다른 과일(즉 인스터스)을 저장할 수 없다.
		컴파일 에러가 발생하므로 실행 자체가 불가능하다. 즉 자료형에는 안전하지만 구현에는 불편함이 있다.
		다른 인스턴스를 저장하기 위해서는 또다른 Box클래스를 생성해야 한다.
		 */
		/*
		아래와 같이 Orange가 아닌 인스턴스를 저장하면 즉시 컴파일 에러가 발생하여 실행할 수 없는 코드가 된다.
...
//		oBox1.store("당도가 20인 오렌지");
		 ...
		 */
		//Orange인스턴스를 저장 후 꺼내는 건 문제없이 실행된다.
		FruitBox fBox1 = new FruitBox(new Orange(20));
		Orange orange3 = (Orange)fBox1.pullOut();
		orange3.showInfo();

		/*
		하지만 오렌지같은 String을 저장하고, 이를 꺼내서 다운캐스팅 하는 과정에서 예외가 발생한다.
		즉 런타임에러가 발생하므로 실행전에는 오류를 예측하기 힘든 단점이 있다.
		 */
