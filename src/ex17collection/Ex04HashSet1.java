...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		/*
		객체저장 : add()메서드를 통해 저장한다. 저장에 성공한 경우 true를 반환한다.
		 */
		...
		/*
		기본클래스의 객체 중복저장 : 기본클래스의 인스턴스인 경우 별도의 오버라이딩 없이도 중복이 제거된다. 
		...
		/*
		이터레이터를 통한 객체 출력 : 순서 없이 저장되므로 출력의 순서도 정할 수 없다.
		 */
		//1. set참조변수를 통해 Iterator인스턴스 생성
		...
		while(itr.hasNext()) {
			...
			/*
			HashSet에 저장 시 모두 Object형으로 자동형변환되어 저장되므로 우리가 직접 정의한 클래스에 대해서만 instanceof를 통해 
			타입을 확인한 후 다운캐스팅하여 출력한다. 만약 다운캐스팅을 하지 않으면 자식멤버에 접근할 수 없으므로 getName을 호출할 수 없다.
			 */
			if(object instanceof Teacher) {
				System.out.println("Teacher객체=>"+((Teacher)object).getName());
			}
			else {
				//기본클래스인 경우 별도의 처리 없이 출력 가능함.
				System.out.println("저장된객체:"+object);
			}
		}
		/*
		포함여부확인 : set에 해당 인스턴스가 저장되어 있는지 확인하여 boolean을 반환한다.
		 */
		System.out.println(set.contains(varString1)?"varString1 있음":"varString1 없음");
		
		/*
		객체 삭제 : set은 인덱스를 사용하지 않으므로 인스턴스의 참조값을 통해 삭제한다.
		 */
		System.out.println(set.remove(varString2)?"varString2 삭제 성공":"varString2 삭제 실패");
		
		/*
		전체 삭제 : List와 동일한 메서드를 사용한다.
		 */
		set.clear();
