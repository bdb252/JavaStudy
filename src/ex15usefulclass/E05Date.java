...
public class E05Date {
	...
	/*
	Calendar 클래스를 통해 얻은 인스턴스를 초(Sec)로 변환한 후 매개변수로 전달된 날짜서식으로 변환한 문자열을 반환한다.
	 */
	public static String myDateFormat(Calendar c, String formatStr) {
		/*
		서버의 표준시인 1970-01-01부터 현재까지의 시간을 초 단위로 반환해주는 메서드로 이를 '타임스탬프'라고 한다.
		 */
		Date date = c.getTime();
	...
	public static void main(String[] args) {
		...
		/*
		Calendar 클래스는 Java의 대표적인 싱글톤 객체이므로 new를 사용하는 대신 유틸리티 메서드인 getInstance()를 통해 참조값을 얻어온다.
		 */
		Calendar calendar = Calendar.getInstance();
		/*
		월을 표현하는 Month, 분을 표현하는 Minute의 첫글자가 동일하므로 구분을 위해 월은 대문자, 분은 소문자를 사용한다.
		 */
		...
		/*
		앞에서 add를 3번 호출했으므로 결과는 아래와 같이 1년 2개월 3일 후의 날짜가 출력된다.
		 */
		String nowStr3 = myDateFormat(calendar, "yyyy년 MM월 dd일");
		System.out.println("1년2개월3일후:"+nowStr3);
		
		/*
		올해 크리스마스까지 남은 날짜를 계산. 여기서는 두 가지 서식의 날짜를 파싱하여 구한 날짜를 인스턴스로 계산한다.
		 */
		//오늘날짜 : 0000-00-00 형식
...
//		System.out.println(today+" "+future);
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			...
			10자리까지는 순수한 초(Sec)이고, 11~13자리는 밀리세컨즈를 의미한다. 즉 100으로 나눈 후 뒤의 3자리를 제거하면 순수한 초가 반환된다. 
			또한 하루는 24*60*60 = 86400초이므로 아래와 같이 남은 날짜를 계산할 수 있다.
			 */
