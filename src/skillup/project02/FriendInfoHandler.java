...
//		System.out.println("## showAllData 호출됨 ##");
		...
		for(int i = 0; i<numOfFriend; i++) {
			/*
			instanceof 연산자 : 클래스와 인스턴스 간의 상속관계가 있는지 확인하여 true/false 를 반환하는 연산자.
							  클래스간의 형변환이 가능한지 확인할 때 사용한다.
			 */
...
//		System.out.println("## showAllData 호출됨 ##");		
		/*
		저장된 인스턴스의 개수만큼 반복해서 각 인스턴스가 원래 어떤 타입이었는지 확인한 후 원래의 타입으로 형변환한다.
		 */
		for(int i = 0; i<numOfFriend; i++) {
			if(myFriends[i] instanceof UnivFriend) {
