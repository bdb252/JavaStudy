...
public class FriendInfoHandler {
	/*
	연락처를 저장하기 위한 인스턴스형 배열을 생성. 고딩/대딩 각각의 배열이 필요하고, 카운트를 위한 변수도 생성한다.
	 */
...
//		Scanner scan = new Scanner(System.in);
		...
		if(choice == 1) {
			...
			/*
			인스턴스 참조값을 배열에 추가. 카운트용 변수를 후위증가 시켜서 0번인덱스에 먼저 입력 후 1증가한다.			
			 */
...
//		System.out.println("## showAllData 호출됨 ##");
		//고딩친구반복: 저장된 정보의 개수만큼 반복해서 전체정보를 출력한다. 4개의 멤버변수를 모두 출력한다.
...
//		Scanner scan = new Scanner(System.in);
		...
		for(int i = 0; i<numOfhighFriend; i++) {
			/*
			문자열 비교를 위한 메서드 중 compareTo()를 사용해서 검색기능 구현.
...
//		Scanner scan = new Scanner(System.in);
		...
		/*
		삭제될 인스턴스의 인덱스를 저장하기 위한 변수. 배열의 인덱스는 0부터 시작이므로 초기값을 -1로 설정한다.
		 */
		...
		//고딩친구반복
		for(int i = 0; i<numOfhighFriend; i++) {
			//삭제할 이름과 일치하는 인스턴스를 찾기 위해 eqauls()를 사용. 일치하면 true를 반환한다.
			if(deleteName.equals(highFriends[i].name)) {
				/*
				일치하는 이름을 찾았다면 해당 인덱스에 null을 입력한다. 
				참조값이 null이라는 것은 참조할 인스턴스가 없다는 의미이므로 삭제라고 볼 수 있다.
				 */
				highFriends[i] = null;
				//삭제된 인스턴스의 인덱스를 저장
				deleteIndexHigh = i;
		...
		if(deleteIndexHigh!=-1) {
			...
			for(int i = deleteIndexHigh; i<numOfhighFriend; i++) {
				/*
				삭제된 인스턴스의 인덱스부터 마지막까지 반복하면서 뒤쪽의 인스턴스를 한칸씩 앞으로 이동한다.
				배열 가운데 null값이 입력되어있으면 정보출력시 NullPointerException이 발생하기 때문이다.
				 */
