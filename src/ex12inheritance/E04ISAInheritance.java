...
class Computer{
	...
	//public Computer() {};
	/*
	매개변수가 있는 인자생성자 추가. 이 경우 디폴트생성자는 추가되지 않는다. 
	 */
	...
	/*
	private으로 선언된 멤버메서드를 외부에서 호출할 수 있도록 public으로 지정하여 선언한 메서드.
	"캡슐화"되었다고 볼 수 있다.
	 */
...
/*
컴퓨터를 상속한 노트북
: 컴퓨터가 가진 기본 계산기능에 휴대성을 확장하여 정의한다. 휴대하기 위해서 배터리가 필요하므로 멤버변수로 확장하였다.
 */
class NotebookComputer extends Computer{
	...
	public NotebookComputer(String name, int initCharge) {
		/*
		부모클래스의 생성자를 호출하는 super()는 생략할 수 있지만. 앞에 다른 문장이 먼저 기술되면 에러가 발생한다.
	...
	public void movingCal() {
		...
		if(battery < 1) {
			/*
			방전된 상태라면 사용이 불가하므로 return을 통해 해당 메서드의 실행을 중단한다.
			 */
			System.out.println("배터리가 방전되어 사용불가");
			return;
		}
		/*
		아래 2개의 메서드는 private으로 선언되었으므로 상속받은 자식클래스에서도 접근할 수 없어 에러가 발생한다.
...
/*
테블릿: 컴퓨터의 기본계산기능과 휴대이용, 그리고 전용펜으로 드로잉할 수 있는 기능까지 추가된 컴퓨터를 표현
 */
class TableNotebook extends NotebookComputer{
	...
	public void write(String penInfo) {
		...
		//등록된 펜인지 문자열 비교를 통해 확인. "A.compareTo(B)"는 일치하는 경우 0을 반환한다.
		if(registPencil.compareTo(penInfo)!= 0) {
			System.out.println("등록된 펜이 아닙니다.");
			return;
		}
		/*
		부모클래스에 정의된 메서드를 호출한다. 상속받은 자식 클래스에서는 이와같이 부모의 멤버메서드를 호출하여 
		자신의 기능처럼 사용할 수 있다. 단 접근이 가능한 지정자로 선언되어야한다.
		super는 생략할 수 있다.
		 */
		super.movingCal(); //부모클래스메서드인지 자식클래스메서드인지 확인 위해 super 붙임
...
public class E04ISAInheritance {
	...
	public static void main(String[] args) {
		...
		noteBook.movingCal();
		noteBook.movingCal();
		noteBook.movingCal(); //배터리가 1씩 차감되므로 여기까지 사용 가능
