...
class Account{
	...
	private int money;
	/*
	이와 같이 개발자가 생성자를 직접 정의하면 디폴트 생성자가 추가되지 않는다. 생성자를 정의하지 않은 경우에만 자동으로 추가된다.
	 */
...
class SavingAccount extends Account{
	/*
	인자생성자에서 부모의 생성자를 호출하기 위한 super(x)를 사용한다. 이때 매개변수가 하나인 부모클래스의 생성자가 호출된다.
	현재 부모클래스에는 매개변수가 하나인 생성자가 유일하므로 만약 아래와 같이 호출하지 않으면 에러가 발생한다.
	 */
	public SavingAccount(int initVal) {
		/*
		이 문장을 삭제하면 에러발생. 삭제하는 경우 super()라는 문장, 즉 매개변수가 없는 디폴트 생성자를 호출하는 문장이 자동으로 추가되는데,
		부모클래스에는 이런 형태의 생성자가 없으므로 에러가 발생함.
		*/
		super(initVal);
	}
	...
	public void showAccountMoney() {
		/*
		잔고확인을 위한 멤버메서드로 부모의 멤버인 money에 직접 접근할 수 없으므로 아래와 같이 getter메서드로 반환받아 출력해야 한다.
		 */
...
//		sa.money= 100000; //에러발생
		...
		/*
		부모클래스를 통해 인스턴스를 생성한다. 하지만 해당 인스턴스 변수를 통해서도 money에 접근할 수 없다.
		현재의 위치가 외부클래스이기 때문이다. 즉 private 멤버는 무조건 클래스 내부에서만 접근을 허용한다.
		 */
