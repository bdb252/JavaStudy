...
디폴트 생성자는 자동으로 삽입되고, 자식쪽에 super()도 자동으로 삽입될 수 있으므로 생략 가능한 코드가 존재한다.
//class AA{
//	public AA() {} //생략가능
//}
//class BB extends AA{
...
: 기존에 정의된 클래스에 메서드와 변수를 추가하여 새로운 클래스를 정의하는 것을 말한다.
  부모클래스를 자식클래스가 상속할 때 extends를 사용한다. 
 */

//부모(상위)클래스: 사람의 일반적인 형태를 추상화
class Man{
	//멤버변수는 private이므로 클래스 내부에서만 접근가능
	private String name;
...
class BusinessMan extends Man{
	...
	public BusinessMan(String name, String company, String position) {
		/*
		상속받은 자식클래스는 부모클래스의 생성자를 호출하여 부모인스턴스가 메모리에 먼저 생성되도록 해야 한다.
		super()는 부모클래스의 생성자를 호출해준다. 만약 super()를 기술하지 않으면 컴파일러에 의해 자동으로 삽입되어 디폴트 생성자를 호출하게 된다.
		 */
		
		//부모클래스의 매개변수 1개인 생성자 호출(메모리에 먼저 생성됨)
		super(name); //이게 주석처리되면 컴파일러에 의해 super()이 자동으로 삽입됨
	...
	public void tellYourInfo() {
		...
		/*
		부모클래스의 멤버변수 name은 private으로 선언되었으므로 상속받은 자식클래스에 접근할 수 없다.
		private멤버는 무조건 자신의 클래스 내부에서만 접근할 수 있다.
		 */
//		System.out.println("이름은 "+name+" 입니다.");
		/*
		직접 접근이 안되므로 public 으로 선언된 메서드를 통해 간접적으로 접근해야 한다.
		 */
		tellYourName();
	}
}
