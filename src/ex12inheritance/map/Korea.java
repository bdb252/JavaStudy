...
public class Korea {
	...
	public void publicMethod(String loc) {
		/*
		public으로 선언된 멤버메서드는 어디서든 접근 가능하다. 동일패키지, 다른패키지, 외부클래스, 상속 등
		모든 영역에서 접근할 수 있다.
		 */
		System.out.println(loc+"publicMethod()호출");
		/*
		default로 선언된 메서드는 패키지가 다른 클래스에서는 호출할 수 없으므로 해당 메서드를 통해 간접호출해야한다.
		 */
	...
	void defaultMethod(String loc) {
		/*
		패키지가 동일할 때만 접근을 허용한다. 패키지가 다른 경우 상속관계가 아니라면 public으로 선언해야 접근할 수 있다.
		 */
