/*
패키지 선언부 : 현재 작성하는 클래스의 위치를 결정하기 위한 선언으로 해당 클래스를 특정 패키지 하위에 묶어 정의한다는 의미
 */
package ex09package;

/*
임포트 선언부 : 현재 작성중인 클래스에서 다른 패키지에 정의된 클래스를 가져다 쓰기 위한 선언으로, 사용할 클래스의 위치를 지정하는 것으로 생각하면 된다.
 */
...
public class E01PackageMain {
	...
	public static void main(String[] args) {
		...
		Java에서 제공하는 클래스 중 대표적으로 Math클래스를 예로 들 수 있는데, 난수생성을 위해 별도의 인스턴스 생성없이 Math.random()을 호출할 수 있다.
		 */
		...
		/*
		클래스 사용 시 패키지명까지 모두 입력한 상태로 메서드를 호출할 수 있다. 이 경우에는 별도로 import가 필요하지 않다. 
		 */
