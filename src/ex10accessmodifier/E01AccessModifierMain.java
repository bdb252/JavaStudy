...
public class E01AccessModifierMain {
	...
	public static void main(String[] args) {
		...
		/*
		private으로 선언된 멤버는 동일한 패키지에 선언하더라도 접근할 수 없다. 유일하게 동일한 클래스 내부에서만 접근을 허용한다.
		 */
		...
		//멤버메서드도 변수와 동일한 접근범위를 가진다.
		//one.privateMethod(); //에러발생
		/*
		private으로 선언된 멤버에 외부클래스에서 접근하려면 default 혹은 public으로 선언되어 외부 접근이 가능한 메서드를 통해
		간접적으로 접근해야 한다. 클래스 내부에서는 접근지정자의 영향을 받지 않으므로 해당 메서드를 통해 접근이 가능해진다.
		 */
		one.defaultMethod();
		one.publicMethod();
		
		/*
		동일한 패키지에 선언된 클래스이므로 import없이 인스턴스 생성이 가능하다. 또한 default 클래스는 동일 패키지내에서 접근할 수 있다.
		 */
		...
		/*
		다른 패키지에 선언된 클래스이므로 인스턴스 생성을 하려면 import선언을 해야한다.
