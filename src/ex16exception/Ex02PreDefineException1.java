...
public class Ex02PreDefineException1 {
	...
	public static void main(String[] args) {
		...
		/*
		예외가 발생할 수 있는 지역을 try로 먼저 묶어주고 예외가 발생되면 JVM(자바가상머신)은 해당 예외에 관련된 인스턴스를 만들어 프로그램쪽으로 전달한다.
		...
		try {
			...
			/*
			예외가 발생하는 지점.
			해당 코드가 실행될 때 예외가 발생하게되고 JVM이 예외 인스턴스를 생성해서 프로그램으로 전달한다.
			 */
			intArr[2] = 300;
			 ...
			 */
			//예외메시지1 : 개발자가 임의로 메시지를 출력
			System.out.println("예외가 발생했어요");
			//메시지2 : 간략한 설명을 출력
			System.out.println("예외메세지:"+e.getMessage());
			//메시지3 : 예외에 대한 디테일한 설명을 출력(가장 많이 사용됨)
			e.printStackTrace();
		}
