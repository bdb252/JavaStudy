...
class MyParent{
	int parentMember;
	void parentMethod() {
		System.out.println("부모의 메서드: parensMethod()");
	}
}
//자식클래스
class MyChild extends MyParent{
	...
	/*
	메서드명은 동일하지만 매개변수가 다르므로 오버로딩으로 정의한 메서드이다. 따라서 자식에서 확장한 멤버가 된다.
	 */
...
public class E06Polymorphism {
	...
	public static void main(String[] args) {
		...
		myChild.childMethod();
		myChild.parentMethod(1000); //오버로딩한 메서드
		myChild.parentMethod(); 	//오버라이딩한 메서드
		
		/*
		이질화: 부모타입의 변수로 자식인스턴스를 참조하는 것을 말한다. 이 경우 자식클래스에서 새롭게 정의한 멤버는 접근할 수 없다.
...
//		parent1.childMember = 1;
		...
		/*
		부모 참조변수로 접근하려면 강제형변환(다운캐스팅)해야 한다. 하지만 아래와 같이 기술하면 에러가 발생한다.
		//(MyChild)myParent.childMember = 1;
		소괄호를 하나 더 추가해서 myParent를 자식 타입으로 형변환 후 멤버변수에 접근해야 한다.
		 */
		((MyChild)myParent).childMember = 1;
		...
		/*
		자식타입으로 형변환 이후에 해당 참조변수를 통해 자식 멤버에 접근한다. 위와 동일한 방식이다.
		 */
		...
		/*
		Java에서 생성한 모든 클래스는 Object클래스를 상속한다. 따라서 모든 인스턴스는 Object의 참조변수로 참조가 가능하다.
		또한 Object 클래스에 정의된 모든 메서드를 별도의 선언 없이 사용할 수 있고, 필요하다면 오버라이딩도 할 수 있다.
		 */
