...
public class DeChild extends DeParent{
	...
	public void study() {
		/*
		부모의 멤버변수 name은 private이므로 자식에서 사용하기 위해 getter을 호출해야 한다.
		 */
//		System.out.println(name+"이(가) 공부한다.");
		System.out.println(getName()+"이(가) 공부한다.");
	}
	/*
	오버라이딩아님: 부모클래스에서 private멤버로 선언하였으므로 자식쪽에서는 보이지 않아 오버라이딩의 대상이 될 수 없다.
	...
	/*
	static정적메서드는 오버라이딩의 대상이 될 수 없다. 
	클래스 외부(메서드영역)에 미리 로드되므로, 오버라이딩 되지않고 호출할 때 각 클래스명을 사용하면 된다.
	 */
