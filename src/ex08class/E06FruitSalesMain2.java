...
class FruitSeller2 {
	/*
	두번째 버전에서는 멤버변수의 초기값을 제거한다. 이 경우 멤버상수는 에러가 발생하므로 final을 제거하여 일반적인 멤버변수로 변경한다.
	변수는 소문자를 사용하므로 이 부분도 변경해준다. ctrl+shift+x(대문자) / ctrl+shift+y(소문자) 
	 */
	int numOfApple;
	int myMoney;
	int apple_price;
	...
	/*
	초기화메서드를 선언하여 인스턴스 생성 후 다양한 형태로 초기화할 수 있다.
	 */
	...
	public int saleApple(int money) {
		//Java는 대소문자를 구분하므로 이부분을 소문자로 변경
