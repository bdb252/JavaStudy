...
그래서 다른 지역의 메서드에서 값이 변경이 있더라고 호출한 메서드의 값에는 전혀 영향을 미치지 않는다.
메모리적 관점에서 보면 스택 영역만 사용하는 케이스로 main과 호출된 메서드는 서로 다른 지역이므로 
...
public class E03CallByValue {
	...
	public static void main(String[] args) {
		...
		/*
		매개변수로 전달된 값은 복사되어 다른 지역의 메서드 내에서만 변경되므로 main 메서드에서는 초기값이 그래도 유지된다. */
	...
	public static void callByValue(int fNum, int sNum) {
		/*
		Swap(스왑) : 두 개의 변수가 할당받은 값을 서로 교환하는 것을 말한다. 교환 시에는 임시변수(temp)가 하나 추가로 필요하다. */
		int temp;
		temp = fNum;
		fNum = sNum;
		sNum = temp;
