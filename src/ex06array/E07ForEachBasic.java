...
배열의 원소를 참조의 형태로 가져와서 사용. 따라서 참조의 목적으로만 사용해야 하고, 값의 변경은 허용하지 않는다.
형식]	
	for(자료형 참조변수 : 배열명){
		실행문장;
		참조변수를 통해 출력;
...
public class E07ForEachBasic {
	...
	public static void main(String[] args) {
		...
		//배열 arr의 원소를 순서대로 하나씩 참조하여 출력하고, 더이상 참조할 원소가 없을 때 for문이 자동으로 종료된다.
		sum = 0;
		for(int e : arr) {
			System.out.print(e+" ");
			sum+=e;
		...
		/*
		참조형태로 각 원소를 인출해서 변수 b에 할당하는 형식이므로 증가연산자를 통해 증가시킨 값은 실제 배열에 적용되지 않는다.
