...
public class E05TwoDimArray01 {
	...
	public static void main(String[] args) {
		...
		※null값 : 아무것도 없는 값을 일컫는다. 즉 빈값을 말한다.
				스페이스와 같은 공백문자 혹은 0과는 다르므로 명확한 의미를 알아야 한다.
		 */
		int [][] arr = {
				{1,2},
				{3,4,5},
		...
		for(int i = 0; i<arr.length; i++) {
			//2차원배열에서는 각 행의 크기를 "배열명[인덱스]"로 사용
			System.out.printf("%d행의 크기: %d\n", i, arr[i].length);
			//각 행의 참조값을 출력
			System.out.printf("%d행 출력:%s\n", i, arr[i]);
		}
		/*
		2차원 배열에서 특정 원소에 접근하기 위해서는 2개의 배열 기호가 필요하다.
		또한 값이 없는 원소, 즉 null로 채워져 있는 원소를 출력하면 예외가 발생한다.(배열의 인덱스 초과하여 발생)
		 */
