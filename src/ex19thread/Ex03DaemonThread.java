...
: 메인쓰레드와 Workimg쓰레드(개발자가 정의한 쓰레드)를 일컫는 것으로 메인쓰레드가 종료되어도 계속 실행되며, 
dead상태가 되어야만 실행이 종료괴는 쓰레드를 말한다. 해당 쓰레드는 main()메서드가 종료된 이후에도 계속 실행되는 것을 볼수있다.
 */
class NormalThread extends Thread{
	...
	/*
	쓰레드 동작중 비정상적인 종료가 될때 InterruptedException이 발생할 수 있으므로, 예외처리를 해야한다.
	여디서는 try~catch로 직접 처리하지 않고, throws해서 호출한 위치로 예외던지기를 하고있다.
	 */
...
: 모든 독립쓰레드가 종료되면 자동으로 종료되는 쓰레드를 말한다.
 메인메서드도 하나의 독립쓰레드이므로, 다른 독립쓰레드가 남아있다면 종료될때까지는 프로그램이 종료되는 것은 아니다.
 주로 프로그램의 보조역할로 배경음악재생, 자동저장 등의 업무를 처리한다. 프로그램이 종료될때 함께 종료되므로 무한루프로 구현한다.
 */
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		...
		/*
		쓰레드로 구현하지 않은 일반적인 클래스의 작업으로 순차적으로 실행되어 1번 인스턴스가 작업을 마쳐야만 2번째가 실행된다.
		 */
		...
		/*
		첫번째 독립쓰레드 생성 및 시작.
		우선순위를 별도로 부여하지 않으므로 5로 지정된다. 쓰레드 인스턴스 생성시 이름을 부여하는 방법으로 아래 2가지가 있다.
		 */
