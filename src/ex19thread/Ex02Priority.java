...
/*
쓰레드의 우선순위
	setPrioority(우선순위): 쓰레드의 우선순위를 지정
...
class MessageSendingThread extends Thread{
	...
	public void run() {
		for(int i = 1; i<=1000; i++) {
			...
			/*
			기본적으로 우선순위가 높은 쓰레드가 먼저 실행되지만 sleep()을 호출하면 실행 중 잠깐씩 블럭상태로 전환되어
...
//		MessageSendingThread tr3 = new MessageSendingThread("세번째");
		...
		/*
		2단계 실행
		:Thread클래스의 정적상수를 이용해서 우선순위르 부여한다.
		순서대로 10,5,1의 우선순위를 부여하게 된다. 상수 대신 정수로 보여해도 된다.
		 */
