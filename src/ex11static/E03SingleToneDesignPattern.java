...
/*
싱글톤 디자인패턴
: 클래스를 설계하는 디자인패턴의 하나로써 하나의 인스턴스 즉 하나의 메모리를 할당한 후 이를 프로그램 전체에서 공유하여 사용할 때 쓰는 패턴이다.
...
class NoSingleTone{
	int instVar;
	/*
	생성자는 거의 대부분 public으로 선언한다. 만약 public으로 선언하지 않으면 클래스 외부 혹은 다른 패키지에서 호출할 수 없기 때문에
...
class SingleTone{
	...
	int shareVar;
	/*
	정의한 클래스와 동일한 타입으로 선언된 정적 멤버변수로 JVM(자바가상머신)에 의해 프로그램이 시작될 때 
	미리 method 영역에 로드되어 사용할 준비를 마치게 된다.
	 */
	private static SingleTone single = new SingleTone();
	/*
	생성자를 private으로 선언하면 클래스 외부에서는 호출할 수 없으므로 new를 통해 인스턴스 생성을 할 수 없다.
	 */
	private SingleTone() {	}
...
//		SingleTone st1 = new SingleTone(); //에러 발생
		...
		/*
		정적메서드이므로 클래스명을 통해 직접 호출할 수 있다. 해당 메서드 호출을 통해 싱글톤 인스턴스의 참조값을 얻어올 수 있다.
		 */
		//여기에서 얻어오는 참조값과 ...
