...
class Singer{
	...
	Java에서 정의하는 모든 클래스는 Object클래스를 상속하므로 이와같이 오버라이딩할 수 있다.
	toString()메서드를 주석처리하면 참조값이 출력됨.
	 */
	@Override
...
/*
인터페이스 정의 : 기능이 없는 추상메서드만 멤버로 정의할 수 있다. 또한 메서드 앞에는 public abstract가 생략되어 있다.
 */
interface ISinger{
	public void dancing();
}
...
public class E02AnonymousClass {
	...
	public static void main(String[] args) {
		...
		/*
		인스턴스의 참조값을 할당받은 참조변수를 출력한다. 이때 오버라이딩한 toString의 반환값이 출력된다.
		만약 toString을 삭제하면 @를 포함한 참조값이 출력된다.
		 */
		...
		접근하고 싶다면 다운캐스팅(강제형변환)을 해야 하지만 익명클래스는 이름이 없기 때문에 다운캐스팅 자체가 불가능하다.
...
//		System.out.println(s3.mic); //에러발생
		...
		/*
		인터페이스를 구현해서 익명클래스를 정의한다. 오버라이딩이 주된 목적이므로, 부모클래스에는 추상메서드만 정의되어있어도 아무 상관 없다.
		 */
